...
---
= Configurez le basculement et le retour à la normale pour les machines virtuelles dans Red Hat OpenShift Virtualization à l'aide de Trident Protect
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Trident Protect doit être installé pour mettre en œuvre les opérations de basculement et de retour à la normale pour les machines virtuelles OpenShift. Consultez les instructions ici link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-installation.html["installer Trident Protect"]


image:redhat-openshift-ocpv-tp-001.png["OCP-v Trident Protect installé dans le namespace trident-protect"]
...
Cette section explique comment configurer un objet AppVault dans Trident Protect en utilisant le stockage d’objets ONTAP S3.
...
image:redhat-openshift-ocpv-tp-002.png["OCP-v AppVault dans l'espace de noms trident-protect"]
...
== Créez une application Trident Protect pour la machine virtuelle de reprise après sinistre dans un nouvel espace de noms
