...
TIP: En plus de la réplication des sauvegardes effectuées avec SCV vers un stockage secondaire, des copies hors site des données peuvent être réalisées sur un stockage objet chez l'un des trois (3) principaux fournisseurs de cloud utilisant NetApp Backup and Recovery pour VM. Pour plus d'informations, veuillez consulter cette offre.link:https://docs.netapp.com/us-en/data-services-backup-recovery/index.html["Documentation de NetApp Backup and Recovery"] .
