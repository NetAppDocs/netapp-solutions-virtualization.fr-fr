...
* Étirez le cluster sur les domaines de pannes avec la synchronisation active SnapMirror . Vérifiez toujours https://mysupport.netapp.com/matrix/#welcome["Outil de matrice d'interopérabilité"] pour les configurations prises en charge.
...
. Pour activer l'option nConnect, accédez à l'interface de ligne de commande sur l'un des nœuds du cluster et tapez pvesm set pvenfs01 --options nconnect=4 où pvenfs01 est l'identifiant de stockage créé à l'étape précédente. Notez que si vous prévoyez d'utiliser la fonctionnalité de trunking, assurez-vous d'utiliser au moins NFS v4.1 et de définir l'option trunkdiscovery. pvesm set pvenfs01 --options vers=4.1,trunkdiscovery
