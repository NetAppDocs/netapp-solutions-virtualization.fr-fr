...
Proxmox Virtual Environment (VE) prend en charge la virtualisation complète basée sur des machines virtuelles et la virtualisation basée sur des conteneurs sur le même hôte. La machine virtuelle basée sur le noyau (KVM) et l'émulateur rapide (QEMU) sont utilisés pour la virtualisation complète de la machine virtuelle. QEMU est un émulateur et virtualiseur de machines open source qui utilise le module noyau KVM pour exécuter du code invité directement sur le processeur hôte. Linux Containers (LXC) permet de gérer les conteneurs comme des machines virtuelles avec une persistance des données après redémarrage. Avec Proxmox VE 9 et versions ultérieures, la prise en charge du registre OCI est incluse pour extraire des images de conteneurs à partir de registres publics et privés. La prise en charge des conteneurs d'applications est ajoutée en tant qu'aperçu technologique dans Proxmox VE 9.
...
Un cluster permet de surveiller les machines virtuelles et les conteneurs et de les redémarrer sur d'autres nœuds en cas de défaillance du nœud hôte. Les machines virtuelles et les conteneurs doivent être configurés pour une haute disponibilité (HA). Les machines virtuelles et les conteneurs peuvent être hébergés sur un sous-ensemble spécifique d'hôtes grâce à la création de groupes d'affinité. La machine virtuelle ou le conteneur est hébergé sur un hôte ayant la priorité la plus élevée. Pour plus d'informations, consultez link:https://pve.proxmox.com/wiki/High_Availability["Gestionnaire HA"]
...
Pour gérer plusieurs clusters Proxmox VE, le produit Proxmox Datacenter Manager est disponible en tant qu'installation séparée. Il offre une interface unique pour gérer plusieurs clusters Proxmox VE et serveurs de sauvegarde Proxmox. Il permet de migrer des machines virtuelles et des conteneurs entre clusters.

Les clients disposant d'un abonnement actif Basic, Standard ou Premium pour leurs serveurs distants Proxmox bénéficient d'un accès au référentiel d'entreprise Proxmox Datacenter Manager et au support technique.
...
Proxmox VE 9 et versions ultérieures prennent en charge le provisionnement fin et la récupération d'espace (UNMAP) avec les types de stockage iSCSI et FC. Pour plus de détails, veuillez consulter link:https://pve.proxmox.com/pve-docs/chapter-pvesm.html#thin_provisioning_and_space_reclamation_support["Stockage Proxmox VE"]
...
Chaque volume de stockage est configuré avec des types de contenu autorisés. Les volumes NAS prennent en charge tous les types de contenu, tandis que la prise en charge SAN est limitée aux images de machines virtuelles et de conteneurs. Le type de stockage de répertoire prend également en charge tous les types de contenu.


NOTE: Les identifiants de connexion SMB sont stockés en clair et ne sont accessibles qu'à l'utilisateur root.
...
Proxmox VE prend en charge à la fois la mise en réseau basée sur l'hôte et les réseaux virtuels à l'échelle du cluster en utilisant des fonctionnalités de mise en réseau Linux natives telles que Linux bridge et Open vSwitch, pour implémenter la mise en réseau définie par logiciel (SDN). Les interfaces Ethernet de l'hôte peuvent être agrégées pour assurer la tolérance aux pannes et une haute disponibilité. Pour d'autres options, consultez link:https://pve.proxmox.com/pve-docs/chapter-sysadmin.html#_choosing_a_network_configuration["Documentation de Proxmox VE"]

image:proxmox-overview-008.png["Réseau lié"]

Les réseaux invités peuvent être configurés au niveau du cluster et les modifications sont répercutées sur les hôtes membres. La séparation est gérée par des zones, des réseaux virtuels et des sous-réseaux. link:https://pve.proxmox.com/pve-docs/chapter-pvesdn.html["Zone"] définit les types de réseau tels que Simple (isolé avec nat source), VLAN (802.1Q - dépendance à un commutateur externe), VLAN Stacking (802.1ad - vlan privé), VXLAN (couche 2 sur couche 3). Pour les réseaux de type VPC), EVPN (VXLAN avec BGP pour créer un réseau multi-cluster de couche 3), etc.
