...
Les machines virtuelles doivent être mises hors tension correctement pendant la fenêtre de maintenance planifiée.  Avant de lancer la migration, vérifiez que toutes les conditions préalables sont remplies :

* Assurez-vous que la machine virtuelle Shift Toolkit fait partie du domaine
* Assurez-vous que le partage CIFS est configuré avec les autorisations appropriées
* Vérifiez que le qtree utilisé pour la migration ou la conversion possède le style de sécurité approprié.
* Créez une machine virtuelle de test à l'aide du Gestionnaire Hyper-V depuis n'importe quel hôte Hyper-V du cluster et placez le fichier VHDx sur le partage CIFS.
* Testez la création d'une machine virtuelle à partir de la machine virtuelle Shift Toolkit en ajoutant des outils de gestion Hyper-V à l'aide de Programmes et fonctionnalités ou de PowerShell : `add-windowsfeature rsat-hyper-v-tools`
