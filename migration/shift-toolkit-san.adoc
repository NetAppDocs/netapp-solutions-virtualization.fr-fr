...
[role="lead"]
Migrez les machines virtuelles des banques de données SAN vers le NAS avant de les convertir avec Shift Toolkit, en utilisant VMware Storage vMotion et Hyper-V Storage Live Migration pour maintenir la continuité de l'activité.
...
.Convertissez les machines virtuelles avec Shift Toolkit*
Une fois les machines virtuelles installées sur la banque de données NFS, Shift Toolkit utilise la technologie NetApp FlexClone pour les convertir de VMware ESXi vers Hyper-V. Les machines virtuelles converties et leurs disques sont ensuite placés sur un partage CIFS accessible par l'hôte Hyper-V.

.Migrer vers un SAN (environnement Hyper-V)*
Après la conversion, utilisez Hyper-V Storage Live Migration pour déplacer les machines virtuelles converties et leurs disques du partage CIFS vers un volume partagé de cluster (CSV) compatible SAN.  Cela vous permet de maintenir votre infrastructure SAN dans l'environnement Hyper-V.
