...
* Les VMDK peuvent être configurés de plusieurs manières : tous les VMDK peuvent résider sur un seul volume (éliminant ainsi le besoin de Storage vMotion), chaque VMDK peut être placé sur son propre volume individuel, ou plusieurs VMDK peuvent être organisés dans un qtree à l’intérieur d’un volume.
+

NOTE: L'outil Shift détecte automatiquement la configuration et sélectionne la méthode de clonage appropriée ainsi que les pilotes de stockage NAS.
