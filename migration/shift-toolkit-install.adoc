...
Le kit d'outils Shift peut être exécuté sur les versions Windows 2019, 2022 et 2025.  Téléchargez le package d'outils Shift depuislink:https://mysupport.netapp.com/site/tools/tool-eula/vm-data-collector["Coffre à outils NetApp"] Décompressez ensuite le package et exécutez le fichier exe pour installer et démarrer le service.  Le kit d'outils Shift propose 2 packages : en ligne (environ 130 Mo) et hors ligne (environ 1,2 Go).  Comme son nom l'indique, le package d'installation en ligne télécharge et installe les prérequis nécessaires depuis Internet.
...
Conseil : Utilisez le package approprié pour le déploiement. Avec le mode hors ligne, il n'est pas nécessaire de modifier les paramètres proxy car tous les prérequis et fichiers requis sont regroupés.
...
Remarque : Installez Shift Toolkit sur une machine virtuelle dédiée.
...
Remarque : Vous devez disposer de 900 Mo d'espace disque disponible pour l'installation.
...
Remarque : Il est recommandé de créer une nouvelle SVM afin de s’assurer que celle-ci réponde aux exigences de la boîte à outils Shift sans avoir à modifier la SVM de production d’une manière qui pourrait être perturbatrice.

Remarque : Pour une migration bidirectionnelle entre VMware et Hyper-V, le nouveau SVM doit avoir les protocoles NFS et SMB activés. Le volume provisionné doit également avoir NFS et SMB activés.
...
Remarque : le chemin de destination doit se trouver sur le même volume que la machine virtuelle source.

Remarque : Attribuez le style de sécurité approprié à l’arbre qtree en fonction du type d’hyperviseur cible et du format de disque.

Remarque : Si les fichiers qcow2 convertis sont destinés à être utilisés avec la virtualisation OpenShift, il existe une exception : ces fichiers peuvent être placés directement sur le volume sans utiliser de qtree.  Pour ce faire, utilisez l'interface graphique de Shift Toolkit ou les API de Shift Toolkit pour convertir les fichiers VMDK au format qcow2 et les placer directement dans le volume.
...
Remarque : le protocole SMB 3.0 doit être activé ; il l’est par défaut.

Remarque : Assurez-vous que la propriété « disponibilité continue » est activée.

Remarque : les stratégies d’exportation pour SMB doivent être désactivées sur la machine virtuelle de stockage (SVM).

Remarque : Le domaine auquel appartiennent le serveur CIFS et les serveurs Hyper-V doit autoriser l’authentification Kerberos et NTLMv2.

Remarque : ONTAP crée le partage avec les autorisations de partage par défaut de Windows, à savoir Tout le monde / Contrôle total.
...
Remarque : Dans la version actuelle, la migration de bout en bout des machines virtuelles est prise en charge uniquement avec Hyper-V, VMware, OpenShift et Oracle Virtualization.

Remarque : Dans la version actuelle, pour KVM comme destination, la conversion VMDK vers qcow2 ou RAW est le seul flux de travail pris en charge. Par conséquent, si KVM est sélectionné dans la liste déroulante, les détails de l’hyperviseur ne sont pas requis. Le disque qcow2 peut être utilisé pour provisionner une machine virtuelle sur des variantes KVM.



== Installation

. Téléchargerlink:https://mysupport.netapp.com/site/tools/tool-eula/vm-data-collector["Boîte à outils de changement de vitesse"] Emballez-le et décompressez-le.
+
...
+
Remarque : Toutes les vérifications préalables sont effectuées et, si les exigences minimales ne sont pas respectées, des messages d'erreur ou d'avertissement appropriés s'affichent.
...
+
Remarque : Le processus d’installation permet de sélectionner l’adresse IP appropriée à l’aide d’une option de liste déroulante si la machine virtuelle est dotée de plusieurs cartes réseau.
...
+
Remarque : Le certificat peut être facilement remplacé par un certificat tiers ou un certificat généré par une autorité de certification.  Il suffit de remplacer le certificat dans le dossier certs.  Le dossier certs se trouve dans <répertoire d'installation>Stockage>Certs.
...
Remarque : Si la machine virtuelle du kit d’outils Shift n’a pas accès à Internet, le programme d’installation hors ligne effectuera les mêmes étapes, mais installera les composants à l’aide des packages inclus dans l’exécutable.

image::shift-toolkit-025.png[Boîte de dialogue affichant l'assistant Shift Toolkit]

Remarque : L'installation peut prendre de 10 à 15 minutes.
...
Remarque : Pour les déploiements sans connexion Internet, téléchargez manuellement le package de mise à niveau commençant par « update » depuis NetApp Toolchest et placez-le dans un dossier désigné (C:\NetApp_Shift).  Ce chemin d'accès au dossier doit être créé et spécifié lors du processus de mise à jour.  Toutes les autres étapes restent identiques à celles de la procédure de mise à niveau en ligne.
