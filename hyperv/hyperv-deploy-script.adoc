...
---
= Migrer des machines virtuelles vers Microsoft Hyper-V à l'aide du script PowerShell
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez le script PowerShell pour migrer des machines virtuelles de VMware vSphere vers Microsoft Hyper-V à l’aide de la technologie FlexClone .  Le script rationalise le processus de migration en se connectant aux clusters vCenter et ONTAP , en créant des instantanés, en convertissant les VMDK en VHDX et en configurant les machines virtuelles sur Hyper-V.
